...
S_f = 10;
D_ss = D;
y_sp = X_ss2;     % For initial time instant
...
X_DATA(i, 2) = X(end, 2);                % Saving the end value (i.e. sample time 1) of the integration. 
X_ss = X(end, :);
t_DATA(i,1) = i;
X_abc = X_DATA(i, 2);
...
K_c2 = (1/Kp2)*(Tau_P2/(Tau_cl2+Tau_D));            % Proportional gain
...
D = D_ss + K_c2*error(i,1);


figure(2)

subplot(3,1,1), plot(t_DATA(1:i,1),X_DATA(1:i,2),'b',t_DATA(1:i,1),y_sp(1:i,1),'r'), xlabel('Time'), ylabel('Substrate Concentration (S)') % legend('Controlled variable', 'Setpoint variable')
